...
@section('title', 'Admin - Buat Peringatan Baru')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white rounded-lg shadow p-6">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">Buat Peringatan Baru</h1>
        ...
        <form action="{{ route('admin.warnings.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="warning_type" id="warning_type" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('warning_type') border-red-500 @enderror" required>
                    ...
                    <option value="Pelanggaran Aturan" {{ old('warning_type') == 'Pelanggaran Aturan' ? 'selected' : '' }}>Pelanggaran Aturan</option>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="subject" id="subject" value="{{ old('subject') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('subject') border-red-500 @enderror" placeholder="Subjek peringatan (misal: Pelanggaran Spam)" required>
                @error('subject')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="message" id="message" rows="5" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('message') border-red-500 @enderror" placeholder="Isi pesan peringatan lengkap" required>{{ old('message') }}</textarea>
                @error('message')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="datetime-local" name="expires_at" id="expires_at" value="{{ old('expires_at') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('expires_at') border-red-500 @enderror">
                @error('expires_at')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-between mt-6">
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition duration-300">
                    <i class="fas fa-paper-plane mr-2"></i> Kirim Peringatan
                </button>
